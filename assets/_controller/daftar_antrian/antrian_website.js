$(function(){
  ...
  $('#no_kartu_pasien').prop('readonly', true);
  $('#nik_pasien').prop('readonly', true);
  $('#layout_norujukan_baru').hide();
  $('#token_humas').hide();
  ...
  form_pendaftaran();
  // get_data_antrian_web_umum_hari_ini();
});
function form_pendaftaran(){
  $('#form_pendaftaran_antrian').show();
  $('#layout_qrcode').hide();
}
function layout_qrcode(){
  $('#form_pendaftaran_antrian').hide();
  $('#layout_qrcode').show();
}
$('#tgl_kunjungan_pasien_baru').change(function(){
  var tgl_kunjungan = $('#tgl_kunjungan_pasien_baru').val();
  // var pecah_tanggal = tgl_kunjungan.split(-");
  var thn_kun_pot   = tgl_kunjungan.substr(0, 4);
  var bln_kun_pot   = tgl_kunjungan.substr(5, 2);
  var tgl_kun_pot   = tgl_kunjungan.substr(8, 2);
  ...
  if (hari < 10) {
    var hari_full = '0'+hari;
  }else{
    var hari_full = hari;
  }
  if (bulan < 10) {
    var bulan_full = '0'+bulan;
  }else{
    var bulan_full = bulan;
  }
  var full_tanggal  = tahun+'-'+bulan_full+'-'+hari_full;
  if (tgl_kunjungan==full_tanggal) {
    if (jam < 11) {
      swal("Anda Masih Bisa Daftar!", {
        icon: "success",
      });
    }else{
      swal("Maaf Pendaftaran Untuk Hari Ini Sudah Tutup!", {
        icon: "warning",
      });
      $('#tgl_kunjungan_pasien_baru').val('');
    }
  }else if (tgl_kunjungan < full_tanggal) {
    swal("Maaf Anda tidak bisa memilih Tanggal di hari Sebelum nya!", {
        icon: "warning",
      });
    $('#tgl_kunjungan_pasien_baru').val('');
  }else{
...
$('#tgl_kunjungan_pasien_lama').change(function(){
  var tgl_kunjungan = $('#tgl_kunjungan_pasien_lama').val();
  // var pecah_tanggal = tgl_kunjungan.split(-");
  var thn_kun_pot   = tgl_kunjungan.substr(0, 4);
  var bln_kun_pot   = tgl_kunjungan.substr(5, 2);
  var tgl_kun_pot   = tgl_kunjungan.substr(8, 2);
...
function get_cabang_outlet(){
  var notoken = token();
  $.ajax({
    type : 'POST',
    url  : base_url()+'index.php/daftar_antrian/daftar_antrian_pasien/cabang_outlet',
    async : false,
    ...
    success : function(data){
      ...
      var option_cabang = '';
      var cabang_kosong = '';
          cabang_kosong += '<option value="" selected="selected"></option>';
      for(d=0; d<data.length; d++){
        option_cabang += '<option value="'+data[d].code_perusahaan+'" >' + data[d].nm_cabang+'</option>';
      }
      $("#cabang_outlet").html(cabang_kosong+option_cabang);
      $('#cabang_outlet_baru').html(cabang_kosong+option_cabang);
    }
  });
}
$('#cabang_outlet_baru').change(function(){
  
});
$("#cabang_outlet").change(function(){
  $('#no_kartu_pasien').prop('readonly', false);
  $('#nik_pasien').prop('readonly', false);
});
function get_cabang_status_pelayanan(){
  var notoken = token();
  $.ajax({
    type : 'POST',
    url  : base_url()+'index.php/daftar_antrian/daftar_antrian_pasien/get_cabang_status_pelayanan',
    async : false,
    data  : 'csrf_klinik_mata_edc='+notoken,
    dataType : 'json',
    success : function(data){
      ...
      var option_cabang = '';
      var cabang_kosong = '';
          cabang_kosong += '<option selected="selected">== || ==</option>';
      for(d=0; d<data.length; d++){
        option_cabang += '<option value="'+data[d].code_perusahaan+'" >' + data[d].nm_cabang+'</option>';
      }
      $("#cabang_outlet").html(cabang_kosong+option_cabang);
...
function tgl_sekarang(){
  ...
  var menit         = date.getMinutes();
  var jam           = date.getHours();
  var tambahan_tanggal = parseInt(tanggal)+1;
  ...
  if (jam < 11) {
    ...
    if (string_tgl.length==1) {
        var tgl_2 = '0'+tambahan_tanggal;
    }else{
        var tgl_2 = tambahan_tanggal;
    }
    $('#tgl_kunjungan_pasien_lama').val(tahun+'-'+bulan_2+'-'+tgl_2);
  }
}
...
$('#daftarkan_antrian').click(function(){
  ...
  $.ajax({
    type      : 'POST',
    url       : base_url()+'index.php/daftar_antrian/daftar_antrian_pasien/daftarkan_pasien_baru_web_umum',
    async     : false,
    data      : 'csrf_klinik_mata_edc='+notoken+'&'+data_form+'&token_url='+notoken,
    dataType  : 'json',
    success   : function(data){
      console.log(data);
      // if (data=="Benar") {
        var token_antrian = data.TOKEN_ANTRIAN;
        $('#code_qr canvas').remove();
        $('#code_qr').qrcode({
          width   : 230,
          height  : 230,
          render  : 'canvas',
...
$('#daftarkan_antrian_px_baru').click(function(){
  ...
  $.ajax({
    ...
    url       : base_url()+'index.php/daftar_antrian/daftar_antrian_pasien/daftarkan_pasien_baru_web_umum',
    async     : false,
    data      : 'csrf_klinik_mata_edc='+notoken+'&'+form_data+'&token_url='+notoken,
    dataType  : 'json',
    success   : function(data){
      console.log(data);
      var token_antrian = data.TOKEN_ANTRIAN;
      // alert(token_antrian);
      // var logo = base_url()+'assets/dist/img/logo.png';
      $('#code_qr canvas').remove();
      $('#code_qr').qrcode({
        width   : 230,
        height  : 230,
  ...
  $.ajax({
    ...
    success   : function(data){
      ...
      if (get_nama_kabupaten!='') {
        // $('#kabupaten').val(get_nama_kabupaten).trigger("change");
        var kabupaten = get_nama_kabupaten.substr(4);
        var kab = get_nama_kabupaten.substr(0,3);
        if (kab=='Kab') {
...
$('#cari_pasien_lama_webumum').click(function(){
  var notoken           = token();
  var form_data_antrian = $('#form_pendaftaran_no_dantrian_pasien_lama').serialize();
  $.ajax({
    type      : 'POST',
    url       : base_url()+'index.php/daftar_antrian/daftar_antrian_pasien/cari_pasien_lama_web_umum',
    async     : false,
    ...
    success   : function(data){
      ...
      if (data=='Data Harus di Isi') {
        ...
        if (data.length==0) {
          ...
          $('#tbody_daftar_pencarian_pasien_antrian').html(html_pasien);
          var html_px_baksos    = '';
          ...
          $('#tbody_daftar_pencarian_pasien_antrian_marketing').html(html_px_baksos);
        }
      }
    }
  });
